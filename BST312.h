...
private:
    ...
    void isItemInTree(TreeNode* t, const ItemType& item, bool& ans);
...
template<class ItemType>
void BST_312 <ItemType>::isItemInTree(TreeNode* t, const ItemType& item, bool& ans) {
    if (t==NULL) {
        ans = false;
        return;
    }
    ...
    if (node->left!=NULL) {
        isItemInTree(node->left, item, ans);
    }
    if (node ->right!=NULL) {
        isItemInTree(node->right, item, ans);
    }
}
